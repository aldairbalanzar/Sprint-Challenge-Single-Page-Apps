...
- [ ]  Explain benefit(s) using `client-side routing`?h

The benefits of client side routing are that instead of fetching data from a server everytime a page needs a refresh, with client side routing we fetch that data once and only render that data according to our state. 
